...
### Site / hosting
- **Navigation added**: Cayman has no built-in nav, so added a small "← Till startsidan · Alla sidor" line at the top of every page except the home page (via docs/_layouts/default.html), plus a new auto-generated overview page (docs/alla-sidor.md) listing every page grouped by section — regenerates itself from site.html_pages, so it can't go stale
