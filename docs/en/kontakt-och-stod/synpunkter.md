---
title: Comments and suggestions
description: "Field manual for the pilot project using light-bucket traps"
---

# Comments and suggestions

This page shows comments and suggestions submitted by project participants. The list is updated on an ongoing basis and shows the current status of each item.

Want to leave a comment or a suggestion? Use this form:

**[Submit a comment or suggestion](https://forms.gle/5Vrf68vXGjDm9eEv9)**

*Please note: this form is in Swedish. If you'd prefer to write in English, just email your comment to [nattflyn@gmail.com](mailto:nattflyn@gmail.com) instead.*

## Submitted comments

<div style="overflow-x: auto;">
<iframe src="https://docs.google.com/spreadsheets/d/e/2PACX-1vSBpji_yqGm2CScCDCXLZ4flLo5_1t8jwpqCmehz2ZiQ6Y_9kOSG-XlxMxf0Kx6XbwYVK-Xx-5rKUaT/pubhtml?gid=502981035&single=true" width="100%" height="500" frameborder="0"></iframe>
</div>
